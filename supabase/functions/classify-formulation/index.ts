...
serve(async (req) => {
  ...
  try {
    const { step, answers, language, confidential_mode, payload_hash, device_id } = await req.json()
    ...
    const antiAbuseBlocked = await requireAntiAbuseGuard(req, supabase, user, device_id, 'classify-formulation')
